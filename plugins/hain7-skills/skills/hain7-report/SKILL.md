...
Accept either a session directory containing `session.meta.json` and `events.jsonl`, a direct `events.jsonl` path, or a spool root with `--latest`. Do not manufacture age, grade, consent, lesson duration, or task version.

Omitting `--context` derives a diagnostic context from the log itself: handle, grade band, curriculum, task version, duration, tool version, and language come from `session.meta.json` and event timestamps. Age, guardian consent, and the notice version are human attestations that no log carries, so a derived context is marked `context_source: auto_derived`, always reports `distribution_allowed: false`, and cannot render a PDF. Use it to inspect a session or debug the pipeline:
...
For any child-facing report, supply the context explicitly:
...
The parent/child-facing PDF follows `references/report-rubric.md` (R1–R9) and `references/perception-rubric.md` (P1–P2) exactly. One A4 page containing:

- the seven-wedge strength map — all seven axes drawn as fully-filled wedges (observed axes sized within the child’s own range, baseline axes sized by their 85–88 band value in the same deep violet as observed petals — no dashed, translucent, or grayed styling anywhere), hub covering the center so nothing collapses;
- 발휘도 for each observed axis: an ipsative 90–100 index (weakest observed = 90, lead = 100) with the mandatory disclosure line — never a percentile, attainment rate, or peer comparison;
- the lead strength hero, each observed axis’s child-readable meaning, two strengths, and one next-session challenge;
- unobserved axes appear as full rows and fully-filled wedges at a disclosed 85–88 baseline band (deterministic per session fingerprint, meaningless within the band) labeled “곧 피어날 힘” — never a named missing-axes list, never faded/grayed styling, never zeros (see references/perception-rubric.md);
- the five verified `EVIDENCE_BASE` citations with URLs, scoped as the basis of the report format;
- “정식 HAIN7/심리검사 아님” and the DEMO DATA badge when synthetic.

Raw 0–4 scores, criterion bands, evidence confidence, coverage, cohort output, and warnings stay in `analysis.json` for operators; none of them are printed on the parent PDF. A session with zero observed axes refuses PDF rendering and returns the “수업이 짧아 기록이 부족해요” replacement notice (R8).

Fonts are bundled: `fonts/NanumGothic-Regular.ttf` (OFL) is the only face used, so line breaks are identical on every machine; `--font`/`HAIN7_FONT` exist only as explicit overrides and there is no silent system-font fallback.

Render the PDF to PNG and visually inspect it before delivery. Reject extra pages, clipped Korean text, overlapping labels, a missing disclosure, any absolute score, or any peer-comparison wording.
