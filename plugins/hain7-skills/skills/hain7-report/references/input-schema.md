...
`session.meta.json` has shipped its handle block under both `identity` and `user`; both are read. `usage` events are billing telemetry — they are indexed with a `U` prefix and the `telemetry` role, and never count as learner evidence.

## Auto-derived context

Running without `--context` derives one from the log. Structural fields come from `session.meta.json` (`u` → `display_id`, `p` → grade band, curriculum, task version, `app_version` → tool version) and event timestamps (`duration_minutes`, `duration_band`, `date`). Language is detected from prompt text.

Age, guardian consent, notice version, and correction contact are never derived. Such a context carries `context_source: auto_derived`, resolves `distribution_allowed: false`, and is rejected for PDF rendering. If the handle contains whitespace or Hangul it may be a real name, so pseudonymity is not attested and the run stops. Auto-derived contexts are for inspection and pipeline debugging, never for a report sent to a child, guardian, or instructor.
