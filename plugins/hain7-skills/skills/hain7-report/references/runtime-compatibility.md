...
- `scripts/test_hain7_signal.py` — unit and CLI behavior, including the auto-context and consent gates.
- `scripts/stress_test_sessions.py` — 28 adversarial spool fixtures: missing `session.meta.json`, the legacy `identity` block, mixed-timezone stamps, overnight spans, unparsable dates, malformed or non-UTF-8 JSONL, unknown event types, and injection payloads in log text. Every case must end in a scored result or a clean refusal; an uncaught traceback is a failure.

Run both after any scorer change, and sweep the real spool with `--latest` before shipping.
...
- `scripts/test_hain7_signal.py`와 `scripts/stress_test_sessions.py`가 모두 통과하고;
